...
class BattleField(tk.Canvas):
    ...
    def remove_targets(self, targets_to_remove=None):
        ...
        for id_ in ids:
            # Не нужно удалять элемент словаря `self.targets`, так как это осуществляется в `Target.destroy()`
    ...
    def remove_bullets(self, bullets_to_remove=None):
        ...
        for id_ in ids:
            # Не нужно удалять элемент словаря `self.bullets`, так как удаление осуществляется в `Ball.destroy()`
            self.bullets[id_].destroy()

    def create_targets(self):
        for _ in range(self.num_targets):
            # Не нужно добавлять элемент словаря `self.targets`, так как это осуществляется в `Target.__init__()`
            Target(self)
    ...
    def catch_victory(self):
        """Завершает раунда и показывает сколько выстрелов потребовалось, чтобы сбить цели."""
        if not self.targets:
            self.catch_victory_job = None
            self.stop()
...
class GunGameApp(tk.Tk):
    ...
    def save(self, event=None):
        ...
        if file_name is not None:
            with open(file_name, 'w') as f:
                # Аргумент `indent` обеспечивает за красивое оформление JSON файла.
    ...
    def pause(self):
        ...
        `'pause'`. Игру можно возобновить с помощью метода `GunGameApp.play()`.
        """
    ...
    def stop(self):
        """Снимает все отложеннве задачи. Отложенным задачам причваиватеся `None`."""
